 ...
 * Guarded so that it never runs in production by accident.
 */
async function main() {
  const e = env();
  if (!e.SEED_DEMO) {
  ...
  console.log("\nDemo data ready.\n");
  console.log(`Guardian:           ${users.parent.email}`);
  console.log(`Co-guardian:        ${users.coGuardian.email}`);
  console.log(`Institution admin:  ${users.institutionAdmin.email}`);
  console.log(`Teacher:            ${users.teacher.email}`);
  console.log(`Password (all):     ${e.DEMO_PASSWORD}`);
  console.log(`Kinder invite code: ${kinder.inviteCode}`);
  console.log(`\nCare Pass links (tokens are only printed here, never stored):`);
  console.log(`  Abuela Rosa (no PIN): ${rosa.url}`);
  console.log(`  Carla (PIN 2468):     ${carla.url}\n`);
}
